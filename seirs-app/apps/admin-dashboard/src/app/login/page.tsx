...
function LoginForm() {
  ...
  const handleCredentials = async (e: FormEvent) => {
    e.preventDefault();
    if (!passwordValid) { setError('Password must be at least 12 characters.'); return; }
    setError('');
    setLoading(true);
    try {
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      required
                      className={`w-full pl-9 pr-3 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#3A7BD5] focus:border-transparent ${
                        password && !passwordValid ? 'border-red-300 bg-red-50' : 'border-gray-200'
                      }`}
                  ...
                  {password && !passwordValid && (
                    <p className="text-xs text-red-500 mt-1 flex items-center gap-1">
                      <AlertCircle size={11} /> Minimum 12 characters required
                    </p>
                  )}
                </div>
