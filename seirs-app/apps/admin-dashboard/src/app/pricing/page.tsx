...
export default function PricingPage() {
  ...
  const field = (key: keyof PricingConfig, label: string, prefix = '', suffix = '', type: 'number' | 'toggle' = 'number') => {
    ...
    if (type === 'toggle') {
      return (
        <div key={key} className="flex items-center justify-between py-4 border-b border-[#F5F0EB] last:border-0">
          <p className="text-sm font-medium text-[#0D1B2A]">{label}</p>
          <button
            onClick={() => setForm({ ...form, [key]: !form[key] })}
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
              form[key] ? 'bg-[#F4600C]' : 'bg-[#0D1B2A]/20'
            }`}
          >
    ...
    return (
      <div key={key} className="flex items-center justify-between py-4 border-b border-[#F5F0EB] last:border-0">
        ...
        <div className="flex items-center gap-1">
          {prefix && <span className="text-sm text-[#0D1B2A]/40">{prefix}</span>}
          <input
            type="number"
            ...
            className="w-28 text-right border border-[#EDE4D9] rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#F4600C] bg-white text-[#0D1B2A]"
          />
          {suffix && <span className="text-sm text-[#0D1B2A]/40">{suffix}</span>}
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="min-h-screen">
      <main className="p-8 max-w-2xl mx-auto">
        <h1 className="text-2xl font-bold text-[#0D1B2A] mb-6">Pricing Controls</h1>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-sm border border-[#EDE4D9] p-6">
              <h2 className="text-xs font-semibold uppercase tracking-wide text-[#0D1B2A]/40 mb-2">Base Rates</h2>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-[#EDE4D9] p-6">
              <h2 className="text-xs font-semibold uppercase tracking-wide text-[#0D1B2A]/40 mb-2">Surge Pricing</h2>
            ...
            <div className="bg-[#0D1B2A] rounded-xl p-6 text-white">
              <p className="text-xs font-semibold uppercase tracking-wide text-white/40 mb-1">Live Preview — 10 km delivery</p>
            ...
            <button
              onClick={handleSave}
              disabled={saving}
              className="w-full bg-[#F4600C] text-white py-3 rounded-xl font-semibold text-sm hover:bg-[#D95209] disabled:opacity-50 transition-colors"
            >
              {saving ? 'Saving…' : saved ? 'Saved!' : 'Save Changes'}
            </button>
          </div>
        ) : (
          <p className="text-[#0D1B2A]/30 text-center py-20">Failed to load config.</p>
        )}
      </main>
    </div>
  );
