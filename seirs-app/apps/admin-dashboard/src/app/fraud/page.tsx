...
const STATUS_COLORS: Record<string, string> = {
  ...
  dismissed: 'bg-[#0D1B2A]/5 text-[#0D1B2A]/50',
    ...
    <div className="min-h-screen">
      <main className="p-8">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-[#0D1B2A]">Fraud Flags</h1>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {data?.flags?.map((flag: any) => (
              <div key={flag.id} className="bg-white rounded-xl border border-[#EDE4D9] shadow-sm p-5">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-[#0D1B2A]/60">{flag.user?.name} — {flag.user?.email}</p>
                    {flag.details && (
                      <pre className="mt-2 text-xs bg-[#F5F0EB] rounded-lg p-2 text-[#0D1B2A]/50 overflow-x-auto border border-[#EDE4D9]">
                        {JSON.stringify(flag.details, null, 2)}
                      </pre>
                    )}
                    <p className="text-xs text-[#0D1B2A]/30 mt-2">
                  ...
                  {flag.status === 'open' && (
                    <div className="flex flex-col gap-2 shrink-0">
                      ...
                      <button
                        ...
                        className="text-xs bg-[#0D1B2A]/5 text-[#0D1B2A]/60 px-3 py-1.5 rounded-lg hover:bg-[#0D1B2A]/10 font-medium transition-colors"
                      >
                        Dismiss
                      </button>
                    </div>
            ...
            {data?.flags?.length === 0 && (
              <div className="text-center py-16 text-[#0D1B2A]/30 bg-white rounded-xl border border-[#EDE4D9]">
