/**
 * Support ticket thread (Chat 5).
 *
 * Renders the ticket conversation between the user and a SEIRS
 * support agent. Reuses the chat 3 features (paperclip attach,
 * fullscreen image viewer) so users can send screenshots of issues
 * directly to support.
 *
 * Refresh cadence: polls every 15s while the thread is open + when
 * the screen gains focus. No WebSocket subscription (support volume
 * is far lower than delivery chat and 15s poll is cheaper than a
...
export default function SupportTicketThreadScreen() {
  ...
  const load = useCallback(async () => {
    try {
      const t = await supportApi.thread(ticketId);
      setThread(t);
    } catch {
      setThread(null);
    }
  ...
  // Poll while the screen is mounted, only if ticket is not closed.
  useEffect(() => {
    if (!thread) return;
  ...
  const sendImage = async (uri: string) => {
    ...
    try {
      ...
      // Support tickets use the same chat_messages table; supportApi.reply
      // takes only text, so for image-only messages we send a short label
      // + the image URL as body. Backend still stores it as a normal reply.
  ...
  const pickCamera = async () => {
    ...
    if (!result.canceled && result.assets?.[0]?.uri) await sendImage(result.assets[0].uri);
  };
  ...
  const attach = () => {
    ...
    Alert.alert(
      ...
      [
        { text: t('chat.attach.camera',  { defaultValue: 'Take photo' }),   onPress: pickCamera  },
        { text: t('chat.attach.gallery', { defaultValue: 'From gallery' }), onPress: pickGallery },
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <View style={styles.emptyWrap}>
                <Text style={{ color: theme.textThird, fontSize: FontSize.sm }}>
                  {t('support.threadEmpty', { defaultValue: 'This thread is empty.' })}
                </Text>
              </View>
            }
            renderItem={({ item }) => {
