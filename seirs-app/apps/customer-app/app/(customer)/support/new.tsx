/**
 * New support ticket form (Chat 5).
 *
 * Three fields: topic (segmented picker), subject (single line),
 * first message (multi-line). Backend enforces the rate limits and
 * inserts the after-hours auto-response, so the client stays thin.
 */
import {
...
const TOPICS: { key: TicketTopic; label: string; icon: any }[] = [
  ...
  { key: 'driver',   label: 'Driver',        icon: 'bicycle-outline' },
...
export default function NewSupportTicketScreen() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      // Replace history so the user cannot re-open the form by tapping back.
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.background }} edges={['top', 'bottom']}>
      ...
      <View style={[styles.header, { backgroundColor: theme.surface, borderBottomColor: theme.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: theme.text }]}>
          {t('support.newTitle', { defaultValue: 'New support ticket' })}
        </Text>
        <View style={{ width: 36 }} />
      </View>
        ...
        <ScrollView contentContainerStyle={{ padding: Spacing.md, gap: Spacing.lg, paddingBottom: 40 }}>
          ...
          <View>
            <Text style={[styles.label, { color: theme.textSecond }]}>
              {t('support.topicLabel', { defaultValue: 'What is this about?' })}
            </Text>
              ...
              {TOPICS.map(tp => {
                ...
                return (
                  <Pressable
                    ...
                    style={[
                      styles.topicChip,
                      { borderColor: active ? theme.primary : theme.border, backgroundColor: active ? `${theme.primary}18` : theme.surface },
                    ]}
                  >
          ...
          <View>
            <Text style={[styles.label, { color: theme.textSecond }]}>
              {t('support.subjectLabel', { defaultValue: 'Subject' })}
            </Text>
            <TextInput
              value={subject}
              onChangeText={setSubject}
              maxLength={200}
              placeholder={t('support.subjectPlaceholder', { defaultValue: 'One-line summary' })}
          ...
          <View>
            <Text style={[styles.label, { color: theme.textSecond }]}>
              {t('support.messageLabel', { defaultValue: 'Describe the issue' })}
            </Text>
            <TextInput
              value={firstMessage}
              onChangeText={setFirstMessage}
              placeholder={t('support.messagePlaceholder', { defaultValue: 'Tell us what happened. Include tracking codes, times, or amounts when relevant.' })}
              placeholderTextColor={theme.textThird}
              multiline
              numberOfLines={6}
          ...
          >
            {submitting
              ...
              : <Text style={styles.submitText}>{t('support.submit', { defaultValue: 'Open ticket' })}</Text>}
          </Pressable>
