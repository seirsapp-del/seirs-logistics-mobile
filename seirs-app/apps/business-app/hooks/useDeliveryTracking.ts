 ...
 *
 * Wires the business app to the backend's TrackingGateway (Socket.io on
...
export function useDeliveryTracking(deliveryId: string | null): TrackingState {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      /**
       * Business sessions live under their own key. The customer app
       * uses 'seirs_user'; reading that here would hand the gateway no
       * token and the socket would connect anonymously, receiving
       * nothing and reporting no error. Falls back to the customer key
       * only so a shared-session build cannot regress to silence.
       */
      const stored =
        (await AsyncStorage.getItem('seirs_business_user').catch(() => null)) ??
        (await AsyncStorage.getItem('seirs_user').catch(() => null));
