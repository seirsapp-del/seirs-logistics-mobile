...
export function Drawer({ visible, onClose }: DrawerProps) {
  ...
  // Items per Master Spec V7 §2.2 (Driver hamburger drawer)
  const items: { icon: string; label: string; path?: string }[] = [
    { icon: 'User',           label: t('drawer.profile',          { defaultValue: 'Profile' }),               path: '/(driver)/profile' },
    { icon: 'TrendingUp',     label: t('drawer.earnings',         { defaultValue: 'Earnings' }),              path: '/(driver)/earnings' },
    { icon: 'Star',           label: t('drawer.ratings',          { defaultValue: 'Ratings' }),               path: '/(driver)/ratings' },
    { icon: 'Calendar',       label: t('drawer.schedule',         { defaultValue: 'Schedule' }),              path: '/(driver)/schedule' },
    { icon: 'Bell',           label: t('drawer.notifications',    { defaultValue: 'Notifications' }),         path: '/(driver)/notification-settings' },
    { icon: 'Globe',          label: t('drawer.language',         { defaultValue: 'Language' }),              path: '/(driver)/language' },
    { icon: 'BookOpen',       label: t('drawer.codeOfConduct',    { defaultValue: 'Driver Code of Conduct' }), path: '/(driver)/privacy' },
    { icon: 'Shield',         label: t('drawer.insurance',        { defaultValue: 'Insurance Partners' }),    path: '/(driver)/help' },
    { icon: 'HelpCircle',     label: t('drawer.help',             { defaultValue: 'Help & FAQ' }),            path: '/(driver)/help' },
    { icon: 'Lock',           label: t('drawer.privacy',          { defaultValue: 'Privacy Policy' }),        path: '/(driver)/privacy' },
    { icon: 'Phone',          label: t('drawer.contact',          { defaultValue: 'Contact Support' }),       path: '/(driver)/help' },
  ];
    ...
    <Modal visible={visible} transparent animationType="none" onRequestClose={onClose}>
      ...
      >
        ...
        <View style={[styles.header, { borderBottomColor: border }]}>
          <Avatar name={user?.name ?? 'Driver'} size={56} />
          <View style={{ flex: 1 }}>
