/**
 * Driver-side support inbox (Chat 5).
 *
 * Mirrors the customer version but scoped to a driver's tickets.
 * Drivers open support when: package damaged in transit, customer
 * unreachable, address unreachable, fare dispute, or account issues.
 */
import {
...
export default function DriverSupportInboxScreen() {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.background }} edges={['top', 'bottom']}>
      ...
      <View style={[styles.header, { backgroundColor: theme.surface, borderBottomColor: theme.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: theme.text }]}>{t('support.title', { defaultValue: 'Support' })}</Text>
        <View style={{ width: 36 }} />
      </View>
          ...
          ListEmptyComponent={
            <View style={styles.emptyWrap}>
              ...
              <Text style={[styles.emptyBody, { color: theme.textSecond }]}>
                {t('support.emptyBodyDriver', { defaultValue: 'Tap below to reach SEIRS support when you need help on the road. We reply 6am–10pm WAT.' })}
              </Text>
            </View>
          }
          renderItem={({ item }) => {
            ...
            return (
              <Pressable
                onPress={() => router.push(`/(driver)/support/${item.id}` as any)}
