...
const IMAGE_SLOTS: Array<Partial<WebsiteContent>> = [
  ...
  // img_driver_portrait: removed 2026-08-14 when the homepage driver band
  // was cut, restored 2026-08-15 for a new home. The founder wanted an image
...
const STORY_ARTICLES: Array<Partial<WebsiteContent>> = [
  ...
  { slug: 'moving-market-traders', title: 'Market traders: every stall is now a citywide shop',
    ...
    body: 'Nigeria\'s markets are engines: Balogun, Onitsha Main Market, Kano\'s Kurmi, and thousands of neighbourhood stalls between them. But a stall only earns from the people who walk past it.\n\nSocial commerce changed the selling: WhatsApp status, Instagram pages, phone orders. What did not change is the delivery problem: traders lose sales daily because "how will it reach you?" has no good answer, and unreliable dispatch eats the profit on the sales they do make.\n\nWith SEIRS a trader books from the stall, the buyer\'s payment is held until delivery so nobody is chasing transfers on trust, and the trader watches the package to the customer\'s door. Saved runs handle the weekly restock. The stall stays where it is: the customers no longer have to.' },
  { slug: 'moving-tailors-fashion', title: 'Tailors and aso-ebi: the last mile of the craft',
    excerpt: 'A tailor\'s reputation travels with the outfit. Getting it there is part of the craft now.',
  ...
  { slug: 'moving-hot-food', title: 'Hot food: for kitchens that live on their name',
    excerpt: 'Food delivery is a trust business. Hot food gets its own category, the right vehicle, and a trip you can watch.',
    coverImageUrl: '/placeholders/move-food.jpg',
    body: 'Nigeria\'s food business runs on buka pride: the amala must land soft, the jollof must arrive with smoke still in it. Every cold delivery is a customer who never orders again.\n\nSmall food businesses: bukas, home kitchens, small chops caterers, mostly cannot afford their own riders, and shared dispatch means the food waits while other errands finish.\n\nSEIRS gives hot food its own category, with the vehicle chosen for the load and the trip live on the map from kitchen to door. Night market runs and owambe caterers work when the city eats, not when an office closes, and you can book at those hours. Good food, good reviews.' },
  ...
  { slug: 'moving-electronics', title: 'Electronics: handled like eggs, tracked like money',
    excerpt: 'Phones and laptops are the most-stolen cargo on the road. Held payment, verified handoffs and custody records protect both ends.',
    coverImageUrl: '/placeholders/move-electronics.jpg',
    body: 'Computer Village moves a nation\'s worth of phones and laptops, and every seller has the same two nightmares: the package that "never arrived", and the buyer who claims it never did.\n\nHigh-value electronics are where delivery trust breaks down: cash-on-delivery scams, swapped devices, riders who vanish. The result is that sellers restrict delivery to buyers they already know, which is no way to grow.\n\nSEIRS was built for exactly this cargo. Payment is held until delivery is confirmed. High-value packages require a verified handoff before the driver can mark them delivered, The fragile-electronics category prices careful handling in, and every step lands in an audit trail both sides can see.' },
  { slug: 'moving-documents', title: 'Documents: paper with a paper trail',
...
const SEED: Array<Partial<WebsiteContent>> = [
  ...
  },
  {
    ...
    slug:  'home_value_2',
    title: 'Pay your way',
    body:  'Card, bank transfer or USSD, handled by a CBN-licensed processor. Earn loyalty points on every delivery.',
    sortOrder: 2,
  },
  ...
  // ── FAQ ──────────────────────────────────────────────────────────────────
  {
    ...
    body:  'Open the SEIRS app and tap your active delivery to watch the driver move on the map. You can also share a tracking link with the person receiving it, so they can follow it without installing anything.',
  ...
  },
  {
    ...
    body:  'Card, bank transfer and USSD. Payments run through a CBN-licensed processor, so SEIRS never sees your full card number. We do not accept cash on delivery.',
    category: 'payments',
    sortOrder: 2,
  },
  {
    ...
    body:  'Yes. Give the person collecting your SEIRS ID (the code in your profile, which looks like SRS-4KQ2M8XZ) and your full name. The partner staff will check both at handoff.',
    category: 'pickup',
    sortOrder: 3,
  },
  ...
  // ── Changelog ────────────────────────────────────────────────────────────
  {
    ...
    body:  '- **Multi-stop**: business senders can now book up to 5 stops per delivery, ordered for you.\n- **Live earnings**: drivers see today / this week / pending / available in one place.\n- **Package categories**: 16 new types from documents to live animals, each with vehicle-safety rules.\n- **Loyalty tiers**: bronze, silver, gold and platinum, each earning points faster than the last.',
    category: 'release',
    sortOrder: 100,
  },
  ...
  // ── First news article ───────────────────────────────────────────────────
  {
    ...
    excerpt: 'Why we are betting that last-mile delivery needs the same kind of shared infrastructure that payments got.',
    body:  'Last-mile delivery in Nigeria is fragmented. Every e-commerce shop has built its own ad-hoc dispatch. Every restaurant has a guy on an okada. Every wholesaler has a keke arrangement.\n\nSEIRS is building one rail every business and every consumer can use. One app for sending, one app for driving, one dashboard for partner stores: shared infrastructure for everything that moves.\n\nWe are starting in Lagos.',
    category: 'news',
    sortOrder: 0,
  },
];
